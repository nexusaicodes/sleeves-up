 ...
 * reader. Renaming a database does not move it, so [RETIRED_DB] is *deleted* on the way past rather
 * than left on disk as a file nothing would ever open again — the same retirement the old work name
 * and the old channel ids get. Carrying the old file forward instead would have meant a migration
 * from its schema, which is code that runs once and is then wrong to delete but impossible to reach.
 *
...
abstract class NudgeSendDatabase : RoomDatabase() {
    ...
    companion object {
         ...
         * Keep this call for as long as any install may still hold the file — nothing reports back
         * that it is gone, and deleting a database that does not exist is a no-op.
         */
        ...
        fun getDatabase(context: Context): NudgeSendDatabase = cached ?: synchronized(this) {
            ...
            // Before the builder, so the old file goes whether or not the new one already exists.
            appContext.deleteDatabase(RETIRED_DB)

            val instance = Room.databaseBuilder(appContext, NudgeSendDatabase::class.java, DB_NAME)
