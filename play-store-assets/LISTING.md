...
| roll up your sleeves · sleeves up! (as an instruction) · let's get to work | The name describes a state, not an order. Nobody has ever rolled their sleeves up for nine hours, so the metaphor carries no measurement — which is exactly what lets it sit on an app where hours grade nothing. Said *at* the user on a cadence it becomes a mascot, and a mascot is the gamification register this app has none of. **Gloss it in exactly two places and nowhere else:** the full description's opening line, and the welcome tour's first page (`welcome_showing_up_title`). **The test is cadence, not surface.** Those two are each a stranger meeting the name once — the tour runs a single time per install and is retired by `welcome_seen` — whereas a notification, a celebration or an empty state recurs for as long as the app is used, and it is the repetition that does the damage. So: never in one of those, and a third gloss anywhere is one too many. |
